...
public class AddressDao implements AddressImpl {
	...
	private final String CREATE_ADDRESS = "INSERT INTO address (street1, street2, city, state, zip, `primary`, personId) VALUES(?,?,?,?,?,?,?)";
	private final String UPDATE_ADDRESS = "UPDATE address SET street1 = ?, street2 = ?,city= ?, state = ?, zip =? Where personId = ? and `primary` = ? ";
	private final String DELETE_ADDRESS = "DELETE FROM address WHERE personId = ? AND `primary` = ?";
